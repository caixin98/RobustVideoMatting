...
class Trainer:
    ...
    def init_datasets(self):
        ...
        if self.args.dataset == 'synthetic':
            self.train_dataset = SyntheticBurstDataset(size=self.args.size, matting_subset = self.args.matting_subset, split = "train")
            self.validate_dataset = SyntheticBurstDataset(size=self.args.size, matting_subset = self.args.matting_subset, split = "val")
        elif self.args.dataset == "synthetic_patch":
            self.train_dataset = SyntheticBurstPatch(patch_size=self.args.patch_size, size=self.args.size, matting_subset = self.args.matting_subset, split = "train")
            self.validate_dataset = SyntheticBurstPatch(patch_size=self.args.patch_size, size=self.args.size, matting_subset = self.args.matting_subset, split = "val")
        ...
        self.train_loader = DataLoader(self.train_dataset, batch_size=self.args.batch_size, sampler=self.train_sampler, num_workers=self.args.num_workers, )
        self.validate_loader = DataLoader(self.validate_dataset, batch_size=self.args.batch_size, num_workers=self.args.num_workers, )
    def init_model(self):
    ...
    def train_mat(self, data, tag):
        ...
        with autocast(enabled=not self.args.disable_mixed_precision):
            pred_fgr, pred_pha = self.model_ddp(comp_burst, trimap_burst)[:2]
        ...
        if self.rank == 0 and self.step % self.args.log_train_images_interval == 0:
            ...
            self.writer.add_image(f'train_{tag}_fgrs_linear', make_grid(true_fgr.flatten(0, 1), nrow=fgrs_linear.size(1)), self.step)
            self.writer.add_image(f'train_{tag}_alpha_burst', make_grid(true_alpha.flatten(0, 1), nrow=alpha_burst.size(1)), self.step)
            self.writer.add_image(f'train_{tag}_comp_burst', make_grid(comp_burst.flatten(0, 1), nrow=comp_burst.size(1)), self.step)
            ...
            with torch.no_grad():
                with autocast(enabled=not self.args.disable_mixed_precision):
                    for data in tqdm(self.validate_loader, disable=self.args.disable_progress_bar, dynamic_ncols=True):
                        ...
                        pred_fgr, pred_pha = self.model_ddp(comp_burst, trimap_burst)
                        if self.args.predict_seq:
                            true_alpha = alpha_burst
                            true_fgr = fgrs_linear
                        else:
                            ...
                            pred_pha = pred_pha[:,0:1]
                        # print("true_alpha.shape, true_fgr.shape", true_alpha.shape, true_fgr.shape)
                        total_loss +=  matting_loss(pred_fgr, pred_pha, true_fgr, true_alpha)
                        batch_size = comp_burst.size(0)
                        total_count += batch_size
                        if sanity_check:
                            break
            avg_loss = total_loss / total_count
            self.writer.add_scalar('val_loss', avg_loss, self.step)
